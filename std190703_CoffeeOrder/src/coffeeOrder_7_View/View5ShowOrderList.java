...
public class View5ShowOrderList extends JFrame implements ActionListener, WindowListener {
	...
	String columnNames[] = {
			"No" ,"메뉴이름", "Size", "시럽", "샷추가", "휘핑크림", "수량", "총액"
	};
	...
	public View5ShowOrderList() {
		super("Show Order List");		
		setLayout(null);
...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	public void mkTbl() {
		...
		List<DTOOrderedMenu> lst = s.orderBucket;
		
		rowData = new Object[6][8];
		...
		for(int i = 0 ; i < 6 ; i++) {
			if(i < lst.size() ) {
				...
				rowData[i][0] = (i+1) + "";
				rowData[i][1] =	dto.getMenuName();
				rowData[i][2] = dto.getSize();
				rowData[i][3] = dto.getSyrup();
				rowData[i][4] = dto.getShot();
				rowData[i][5] = dto.getWhip();
				rowData[i][6] = dto.getAmount();
				rowData[i][7] = ( s.ctrl.menuPrice(dto.getMenuName(), dto.getSize()) * Integer.parseInt(dto.getAmount()+"") );
			}else {
				rowData[i][0] = " ";
				rowData[i][1] = " ";
				rowData[i][2] = " ";
