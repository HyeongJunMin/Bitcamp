...
public class FrameInsertPitcher extends JFrame implements WindowListener, ActionListener, MouseMotionListener {
	public FrameInsertPitcher() {
		super("Insert Pitcher");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if( e.getSource().equals(btn1) ) {
			try {
				...
				PitcherDTO p = new PitcherDTO(TeamDAOSingleton.team.size() + 1, name, age, height, win, lose, defenceR);
				TeamDAOSingleton.team.put(TeamDAOSingleton.team.size() + 1, p);
				JOptionPane.showMessageDialog(null, name + "선수 입력 성공!");
				FncFileIO.syncListToFile();
				MainFrame.l.add(p.toString());
			} catch (Exception ee) {
