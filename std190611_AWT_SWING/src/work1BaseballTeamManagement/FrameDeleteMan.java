...
public class FrameDeleteMan  extends JFrame implements WindowListener, ActionListener, MouseMotionListener{
	public FrameDeleteMan() {
		super("Delete Player");
			...
			try {
				...
				while( it.hasNext() ) {
					...
					if ( TeamDAOSingleton.team.get(key).getName().equals(name) ) {
						MainFrame.l.remove(key-1);
						TeamDAOSingleton.team.remove(key);
						JOptionPane.showMessageDialog(null, name + " 선수 삭제 완료.");
